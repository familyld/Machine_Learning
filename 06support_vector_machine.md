...
KKT条件说明了，对任何一个样本来说，要么对应的拉格朗日因子为0，要么函数间隔等于1（即式（1）的约束条件取等号）。如果拉格朗日因子为0，则这个样本对式（6）毫无贡献，不会影响到模型；如果函数间隔为1，则表明这个样本位于最大间隔边界上，是一个支持向量。它揭示了SVM的一个重要性质：**最终模型只与支持向量有关，因此训练完成后，大部分的训练样本都不需保留**（支持向量被保留下来用于计算新样本的预测值，也即式（6））。
...
软间隔则**允许某些样本不满足约束**，即使存在误分类也可以。此时目标函数可以重写为：
...
而 $C$ 则是一个大于0的常数，当 $C$ 趋于无穷大时，式（12）等效于带约束的式（1），因为此时对误分类的惩罚无限大，也即要求全部样本分类正确。**当 $C$ 去有限值时，允许某些样本分类错误**。
...
实际任务中**最常用的是hinge损失**，这里就以hinge损失为例，替代0/1损失函数，此时对偶问题式（12）可以重写为：
