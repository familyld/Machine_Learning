...
- **对偶问题**：求取最大间隔等价于怎样的对偶问题？KKT条件揭示出支持向量机的什么性质？如何用SMO算法进行高效求解？
...
$$\max_{\mathbf{w},b} \frac{1}{2} \Vert \mathbf{w} \Vert^2 \quad s.t. \quad y_i(\mathbf{w}^T\mathbf{x}+b) \geq 1, \quad  i=1,2,...,m\qquad(1)$$
...
首先为式（1）的每条约束添加拉格朗日乘子 $a_i \geq 0$（m个样本就有m条约束），得到该问题的拉格朗日函数：
